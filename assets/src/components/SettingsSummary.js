...
const SettingsSummary = ({ settings, postTypes, statusChoices }) => {
  ...
  const getTimeOffsetSummary = () => {
    ...
    if (settings.time_offset_days > 0)
      parts.push(`${settings.time_offset_days} ${__('days', 'post-duplicator')}`)
    if (settings.time_offset_hours > 0)
      parts.push(
        `${settings.time_offset_hours} ${__('hours', 'post-duplicator')}`
      )
    if (settings.time_offset_minutes > 0)
      parts.push(
        `${settings.time_offset_minutes} ${__('minutes', 'post-duplicator')}`
      )
    if (settings.time_offset_seconds > 0)
      parts.push(
        `${settings.time_offset_seconds} ${__('seconds', 'post-duplicator')}`
      )
